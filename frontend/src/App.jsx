...
import {
  Brain, Sun, Moon, Lock, Mic, MicOff, Send, Volume2, VolumeX,
  LayoutDashboard, LineChart as LineChartIcon, BookOpen, MessageSquare,
  Search as SearchIcon, Sparkles, Download, X, FolderOpen, FileSpreadsheet,
  FileJson, FileText, CheckCircle2, AlertTriangle, Lightbulb, ListChecks,
  ShieldCheck, Hash, Link2, BarChart2
} from "lucide-react";
...
const globalStyles = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .listen-btn:hover { opacity: 0.8; }
  .fun-tab:hover { opacity: 0.85; }
  .pii-row { background: rgba(239,68,68,0.03); }
  .pii-row:hover { background: rgba(239,68,68,0.07) !important; }
`;
const styleEl = document.createElement("style");
styleEl.textContent = globalStyles;
document.head.appendChild(styleEl);
...
const FileTypeIcon = ({ name, size=16, color }) => {
  if (!name) return <FileText size={size} color={color || "#94A3B8"} />;
  const ext = name.split(".").pop().toLowerCase();
  if (ext === "csv") return <FileSpreadsheet size={size} color={color || "#10B981"} />;
  if (ext === "xlsx" || ext === "xls") return <FileSpreadsheet size={size} color={color || "#22C55E"} />;
  if (ext === "json") return <FileJson size={size} color={color || "#F59E0B"} />;
  return <FileText size={size} color={color || "#94A3B8"} />;
};
...
const PIIBadge = () => (
  <span title="PII — Personally Identifiable Information. Values are masked." style={{
...
export default function App() {
  ...
  const [colInsight, setColInsight] = useState("");
  const [colInsightLoading, setColInsightLoading] = useState(false);
  const [colInsightIsPII, setColInsightIsPII] = useState(false);
  const [summary, setSummary] = useState(null);
  const [metadata, setMetadata] = useState([]);
  const [insights, setInsights] = useState([]);
  const [recommendations, setRecommendations] = useState([]);
  ...
  const [uploadError, setUploadError] = useState("");
  const [messages, setMessages] = useState([
    { role:"model", content:"Welcome to MetaMind AI!\n\nUpload a CSV, Excel (.xlsx/.xls), or JSON dataset to get started. I'll analyze your data, auto-detect sensitive columns, and generate an intelligent data dictionary.\n\nPII columns (names, emails, phones, etc.) are automatically masked before any AI processing.\n\nYou can also use the mic button to ask questions by voice!", chartData:null },
  ]);
  const [chatLoading, setChatLoading] = useState(false);
  const [dragOver, setDragOver] = useState(false);
  const [isListening, setIsListening] = useState(false);
  ...
  const handleUpload = async (f) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setSensitiveColumns(data.sensitiveColumns||[]);
        const piiNote = data.sensitiveColumns?.length > 0
          ? `\n\n${data.sensitiveColumns.length} PII column(s) detected and masked: ${data.sensitiveColumns.join(", ")}`
          : "\n\nNo sensitive PII columns detected.";
        setMessages([{ role:"model", content:`Dataset loaded successfully!\n\n${data.summary.datasetName}\n${data.summary.datasetPurpose||""}\n\n${data.summary.totalRows} rows x ${data.summary.totalColumns} columns - Quality: ${data.summary.qualityScore}%${piiNote}\n\nAsk me anything — type or use the mic button!`, chartData:null }]);
        setTab("dashboard"); setFunTab(null);
  ...
  const handleDrop = (e) => {
    ...
    if (["csv","xlsx","xls","json"].includes(ext)) { setFile(f); handleUpload(f); }
    else setUploadError("Please drop a CSV, Excel, or JSON file");
  };
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/chat", {
        method:"POST", headers:{"Content-Type":"application/json"},
        body: JSON.stringify({ question, chatHistory: [], sessionId }),
      });
      const data = await res.json();
      setMessages(prev=>[...prev,{ role:"model", content:data.answer, chartData:data.chartData||null }]);
    } catch {
  ...
  const fetchColInsight = async (colName) => {
    setSelectedCol(colName); setColInsight(""); setColInsightLoading(true); setColInsightIsPII(false);
    try {
      const res = await fetch("http://localhost:5000/column-insight", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ columnName:colName, sessionId }) });
      const data = await res.json();
      setColInsight(data.insight);
      setColInsightIsPII(data.isPII || false);
    } catch { setColInsight("Could not load insight."); }
    setColInsightLoading(false);
  };
  ...
  const downloadCSV = () => {
    if (!metadata.length) return;
    const rows = [["Column","Type","PII","Description","Business Meaning","Relationship","Missing %","Unique"],
      ...metadata.map(m=>[
        m.column, m.type, m.isPII?"Yes":"No",
  ...
  const fetchDetective = async () => {
    setDetectiveLoading(true); setFunTab("detective"); setTab(null);
    try {
      const res = await fetch("http://localhost:5000/detective", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ sessionId }) });
      const data = await res.json();
      setDetectiveReport(data.report);
    } catch { setDetectiveReport("Detective is unavailable. Make sure server is running!"); }
    setDetectiveLoading(false);
  ...
  const fetchHoroscope = async () => {
    setHoroscopeLoading(true); setFunTab("horoscope"); setTab(null);
    try {
      const res = await fetch("http://localhost:5000/horoscope", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ sessionId }) });
      const data = await res.json();
      setHoroscope(data.horoscope);
    } catch { setHoroscope("The stars are unclear. Make sure server is running!"); }
    setHoroscopeLoading(false);
  ...
  const filteredMeta = metadata
    .filter(m => m.column.toLowerCase().includes(search.toLowerCase()))
    .filter(m => showOnlyPII ? m.isPII : true);
  ...
  const TabItem = ({ id, label, icon }) => (
    <div className="tab-item" onClick={()=>{ setTab(id); setFunTab(null); }} style={{
      ...
      <span style={{ display:"flex", color:tab===id&&!funTab?t.accent2:t.sub }}>{icon}</span>
      <span style={{ fontSize:"13px", fontWeight:tab===id&&!funTab?600:400, color:tab===id&&!funTab?t.accent2:t.sub }}>{label}</span>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:"100vh", background:t.bg, color:t.text, display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ height:"56px", borderBottom:`1px solid ${t.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 28px", backdropFilter:"blur(20px)", background:dark?"rgba(6,8,15,0.8)":"rgba(248,247,255,0.8)", position:"sticky", top:0, zIndex:100 }}>
        <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
          <div style={{ width:"32px", height:"32px", borderRadius:"10px", background:"#8B5CF6", display:"flex", alignItems:"center", justifyContent:"center" }}><Brain size={18} color="#fff" /></div>
          <span style={{ fontFamily:"'JetBrains Mono',monospace", fontSize:"17px", fontWeight:700, color:t.text }}>MetaMind<span style={{ color:t.accent }}> AI</span></span>
          <div style={{ width:"1px", height:"16px", background:t.border, margin:"0 6px" }} />
          <span style={{ fontSize:"12px", color:t.muted }}>Intelligent Data Dictionary</span>
        </div>
        <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
          {summary && (
            <div style={{ display:"flex", alignItems:"center", gap:"6px", padding:"4px 12px", background:t.card, border:`1px solid ${t.border}`, borderRadius:"8px" }}>
              ...
              <span style={{ fontSize:"12px", color:t.sub, maxWidth:"200px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{summary.datasetName}</span>
              {summary.fileType && (
                <span style={{ fontSize:"10px", fontFamily:"'JetBrains Mono',monospace", color:fileTypeColor(file?.name), fontWeight:700, padding:"1px 6px", background:fileTypeColor(file?.name)+"15", borderRadius:"4px" }}>{summary.fileType}</span>
              )}
            </div>
          )}
          {summary && sensitiveColumns.length > 0 && (
            <div style={{ display:"flex", alignItems:"center", gap:"5px", padding:"4px 10px", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:"8px" }} title={`PII Columns: ${sensitiveColumns.join(", ")}`}>
              <Lock size={12} color="#EF4444" />
              <span style={{ fontSize:"11px", color:"#EF4444", fontWeight:600 }}>{sensitiveColumns.length} PII masked</span>
            </div>
          )}
          <button onClick={()=>setDark(!dark)} style={{ display:"flex", alignItems:"center", gap:"6px", padding:"6px 14px", border:`1px solid ${t.border}`, borderRadius:"8px", background:t.card, color:t.sub, cursor:"pointer", fontSize:"12px", fontWeight:500 }}>
      ...
      <div style={{ display:"flex", flex:1, overflow:"hidden" }}>
        ...
        <div style={{ width:"240px", borderRight:`1px solid ${t.border}`, padding:"20px 12px", display:"flex", flexDirection:"column", gap:"16px", background:dark?"rgba(6,8,15,0.6)":"rgba(255,255,255,0.5)", backdropFilter:"blur(20px)", overflowY:"auto" }}>
          <div style={{ padding:"4px" }}>
            ...
            <div onDragOver={(e)=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)} onDrop={handleDrop}
              ...
              style={{ padding:"18px 12px", border:`2px dashed ${dragOver?t.accent:t.border}`, borderRadius:"12px", textAlign:"center", cursor:"pointer", transition:"all 0.2s", background:dragOver?`${t.accent}10`:"transparent", marginBottom:"10px" }}>
              <input ref={fileInputRef} type="file" accept={ACCEPTED_FORMATS} onChange={e=>setFile(e.target.files[0])} style={{ display:"none" }} />
              <div style={{ display:"flex", justifyContent:"center", marginBottom:"6px", color:file?fileTypeColor(file.name):t.muted }}>
                {file ? <FileTypeIcon name={file.name} size={26} /> : <FolderOpen size={26} />}
              </div>
              {file
                ? <div style={{ fontSize:"11px", color:fileTypeColor(file.name), fontWeight:600, wordBreak:"break-all" }}>{file.name}</div>
                : <>
                    <div style={{ fontSize:"11px", color:t.muted, marginBottom:"4px" }}>Drop file or click</div>
                    <div style={{ fontSize:"10px", color:t.muted, opacity:0.6 }}>CSV · Excel · JSON</div>
                  </>
              }
            </div>
            {uploadError && <div style={{ fontSize:"11px", color:t.accentRed, padding:"8px 10px", background:`${t.accentRed}12`, border:`1px solid ${t.accentRed}25`, borderRadius:"8px", marginBottom:"10px", lineHeight:"1.5" }}>{uploadError}</div>}
            <button onClick={()=>handleUpload()} disabled={loading||!file} style={{ width:"100%", padding:"10px", border:"none", borderRadius:"10px", background:loading||!file?"rgba(139,92,246,0.3)":"#8B5CF6", color:"#fff", cursor:loading||!file?"not-allowed":"pointer", fontWeight:600, fontSize:"13px" }}>
              {loading ? "Analyzing..." : "Analyze"}
            </button>
            ...
            {summary && (
              <>
                <div style={{ marginTop:"12px", padding:"10px 12px", background:t.card, border:`1px solid ${t.border}`, borderRadius:"10px" }}>
                  ...
                  <div style={{ height:"4px", background:t.border, borderRadius:"4px", overflow:"hidden" }}>
                    <div style={{ height:"100%", width:`${summary.qualityScore}%`, background:t.accent, borderRadius:"4px", transition:"width 1.2s ease" }} />
                  </div>
                </div>
                {sensitiveColumns.length > 0 && (
                  <div style={{ marginTop:"8px", padding:"8px 12px", background:"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:"10px" }}>
                    <div style={{ display:"flex", alignItems:"center", gap:"5px", fontSize:"10px", color:"#EF4444", fontWeight:600, marginBottom:"4px", fontFamily:"'JetBrains Mono',monospace" }}><Lock size={11} /> PII PROTECTED</div>
                    <div style={{ fontSize:"11px", color:t.muted, lineHeight:"1.5" }}>{sensitiveColumns.join(", ")}</div>
                  </div>
                )}
              </>
            )}
          ...
          {summary && (
            <div>
              <div style={{ fontSize:"10px", fontFamily:"'JetBrains Mono',monospace", color:t.muted, letterSpacing:"0.1em", marginBottom:"8px", paddingLeft:"4px" }}>NAVIGATION</div>
              <div style={{ display:"flex", flexDirection:"column", gap:"2px" }}>
                <TabItem id="dashboard" icon={<LayoutDashboard size={16} />} label="Dashboard" />
                <TabItem id="visuals"    icon={<LineChartIcon size={16} />} label="Visualizations" />
                <TabItem id="dictionary" icon={<BookOpen size={16} />} label="Data Dictionary" />
                <TabItem id="chat"       icon={<MessageSquare size={16} />} label="AI Assistant" />
                <div className="tab-item" onClick={fetchDetective} style={{
                  display:"flex", alignItems:"center", gap:"10px", padding:"10px 14px", borderRadius:"10px", cursor:"pointer",
                  background: funTab==="detective" ? "rgba(239,68,68,0.15)" : "transparent",
                  borderLeft: funTab==="detective" ? "2px solid #EF4444" : "2px solid transparent",
                }}>
                  <span style={{ display:"flex", color:"#EF4444" }}><SearchIcon size={16} /></span>
                  <span style={{ fontSize:"13px", fontWeight:funTab==="detective"?600:400, color:"#EF4444" }}>Data Detective</span>
                </div>
                <div className="fun-tab tab-item" onClick={fetchHoroscope} style={{
                  display:"flex", alignItems:"center", gap:"10px", padding:"10px 14px", borderRadius:"10px", cursor:"pointer",
                  ...
                  <span style={{ display:"flex", color:"#A78BFA" }}><Sparkles size={16} /></span>
                  <span style={{ fontSize:"13px", fontWeight:funTab==="horoscope"?600:400, color:"#A78BFA" }}>Data Horoscope</span>
                </div>
              </div>
            </div>
          ...
          {summary && (
            <div>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"1px" }}>
                {[
                  ["Rows",    summary.totalRows,                         t.accentBlue],
                  ["Columns", summary.totalColumns,                      t.accent2],
                  ["Numeric", summary.numericColumns,                    t.accentGreen],
                  ["Text",    summary.textColumns,                       t.accentAmber],
                  ["PII",     `${summary.sensitiveColumnsCount||0} cols`,t.accentRed],
                  ["Dupes",   summary.duplicateRows,                     t.accentRed],
                  ["Missing", summary.missingPercentage+"%",             t.accentAmber],
                ].map(([l,v,c])=>(
                  <div key={l} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"6px 10px", borderRadius:"8px" }}>
                    <span style={{ fontSize:"12px", color:t.sub }}>{l}</span>
                    <span style={{ fontSize:"12px", fontFamily:"'JetBrains Mono',monospace", color:c, fontWeight:600 }}>{v}</span>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"24px 28px" }}>
          ...
          {!summary && (
            <div className="fade-up" style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", height:"100%", textAlign:"center", padding:"60px 40px" }}>
              <div style={{ width:"72px", height:"72px", borderRadius:"20px", background:"#8B5CF6", display:"flex", alignItems:"center", justifyContent:"center", marginBottom:"24px" }}><Brain size={32} color="#fff" /></div>
              <h1 style={{ fontSize:"32px", fontWeight:800, color:t.text, marginBottom:"12px" }}>Intelligent Data Dictionary</h1>
              <p style={{ color:t.sub, fontSize:"15px", maxWidth:"520px", lineHeight:"1.7", marginBottom:"12px" }}>Upload any dataset. AI automatically analyzes fields, data types, relationships, and generates human-readable descriptions with interactive visualizations.</p>
              <div style={{ display:"flex", gap:"8px", marginBottom:"32px" }}>
                {[[<FileSpreadsheet size={14} />,"CSV"],[<FileSpreadsheet size={14} />,"Excel"],[<FileJson size={14} />,"JSON"]].map(([icon,label])=>(
                  <div key={label} style={{ padding:"6px 14px", background:t.card, border:`1px solid ${t.border}`, borderRadius:"8px", fontSize:"12px", color:t.sub, display:"flex", alignItems:"center", gap:"5px" }}>
                    <span style={{ display:"flex" }}>{icon}</span><span>{label}</span>
                  </div>
                ))}
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", maxWidth:"520px" }}>
                {[
                  [<SearchIcon size={20} />,"AI Field Analysis","Auto-detect types, relationships, and anomalies"],
                  [<BarChart2 size={20} />,"Smart Visualizations","Interactive charts generated from your data"],
                  [<ShieldCheck size={20} />,"PII Auto-Masking","Names, emails, phones masked before AI processing"],
                  [<Mic size={20} />,"Voice AI Assistant","Talk to your data — ask by voice, hear answers!"],
                  [<SearchIcon size={20} />,"Data Detective","Investigate your data quality issues"],
                  [<Sparkles size={20} />,"Data Horoscope","ML algorithm recommendations for your dataset"],
                ].map(([icon,title,desc])=>(
                  <div key={title} style={{ padding:"16px", background:t.card, border:`1px solid ${t.border}`, borderRadius:"14px", textAlign:"left" }}>
                    <div style={{ marginBottom:"8px", color:t.accent2, display:"flex" }}>{icon}</div>
                    <div style={{ fontSize:"13px", fontWeight:600, color:t.text, marginBottom:"4px" }}>{title}</div>
                    <div style={{ fontSize:"12px", color:t.muted, lineHeight:"1.5" }}>{desc}</div>
                  </div>
                ))}
          ...
          {summary && tab==="dashboard" && !funTab && (
            <div className="fade-up" style={{ display:"flex", flexDirection:"column", gap:"20px" }}>
              <div style={{ background:dark?"rgba(139,92,246,0.08)":"rgba(139,92,246,0.06)", border:`1px solid ${t.borderAccent}`, borderRadius:"18px", padding:"24px 28px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:"16px" }}>
                  <div style={{ flex:1 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"8px" }}>
                      <div style={{ fontSize:"10px", fontFamily:"'JetBrains Mono',monospace", color:t.muted, letterSpacing:"0.1em" }}>DATASET IDENTIFIED</div>
                      {summary.fileType && <span style={{ fontSize:"10px", fontFamily:"'JetBrains Mono',monospace", color:fileTypeColor(file?.name), fontWeight:700, padding:"1px 7px", background:fileTypeColor(file?.name)+"18", borderRadius:"4px", border:`1px solid ${fileTypeColor(file?.name)}30` }}>{summary.fileType}</span>}
                    </div>
                    <h2 style={{ fontSize:"22px", fontWeight:800, color:t.text, marginBottom:"10px" }}>{summary.datasetName}</h2>
                    <p style={{ color:t.sub, fontSize:"14px", lineHeight:"1.7", maxWidth:"680px" }}>{summary.datasetPurpose}</p>
                  </div>
                ...
                {sensitiveColumns.length > 0 && (
                  <div style={{ marginTop:"12px", padding:"10px 14px", background:"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:"10px", display:"flex", alignItems:"center", gap:"10px", flexWrap:"wrap" }}>
                    <span style={{ fontSize:"12px", color:"#EF4444", fontWeight:600, display:"flex", alignItems:"center", gap:"5px" }}><Lock size={13} /> PII Columns Auto-Masked:</span>
                    {sensitiveColumns.map(col=>(
                      <span key={col} style={{ fontSize:"11px", fontFamily:"'JetBrains Mono',monospace", padding:"2px 8px", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.25)", borderRadius:"5px", color:"#EF4444" }}>{col}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              {insights.length>0 && (
                <Card>
                  <SectionTitle icon={<Brain size={15} />} label="AI INSIGHTS" />
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))", gap:"10px" }}>
                    {insights.map((ins,i)=>(
                      <div key={i} style={{ padding:"12px 14px", background:dark?"rgba(139,92,246,0.06)":"rgba(139,92,246,0.04)", border:`1px solid ${t.borderAccent}`, borderRadius:"10px", fontSize:"13px", color:t.text, lineHeight:"1.6" }}>
                        <span style={{ color:t.accent, marginRight:"8px", fontSize:"12px" }}>▸</span>{ins}
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"16px" }}>
                {recommendations.length>0 && (
                  <Card>
                    <SectionTitle icon={<CheckCircle2 size={15} />} label="RECOMMENDATIONS" color={t.accentGreen} />
                    {recommendations.map((r,i)=><div key={i} style={{ fontSize:"13px", color:t.text, marginBottom:"10px", lineHeight:"1.6", paddingLeft:"12px", borderLeft:`2px solid ${t.accentGreen}` }}>{r}</div>)}
                  </Card>
                )}
                {useCases.length>0 && (
                  <Card>
                    <SectionTitle icon={<Lightbulb size={15} />} label="USE CASES" color={t.accentAmber} />
                    {useCases.map((u,i)=><div key={i} style={{ fontSize:"13px", color:t.text, marginBottom:"10px", lineHeight:"1.6", paddingLeft:"12px", borderLeft:`2px solid ${t.accentAmber}` }}>{u}</div>)}
                  </Card>
                )}
              </div>
          ...
          {summary && tab==="visuals" && !funTab && (
            <div className="fade-up" style={{ display:"flex", flexDirection:"column", gap:"20px" }}>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"20px" }}>
                <Card>
                  <SectionTitle icon={<BarChart2 size={15} />} label="DATA TYPE DISTRIBUTION" />
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={typeDistData} cx="50%" cy="50%" outerRadius={82} dataKey="value" label={({name,value})=>`${name}:${value}`} labelLine={false} fontSize={10}>
                        {typeDistData.map((e,i)=><Cell key={i} fill={e.color} />)}
                ...
                <Card>
                  <SectionTitle icon={<SearchIcon size={15} />} label="DATA COMPLETENESS" />
                  <ResponsiveContainer width="100%" height={220}>
                    <PieChart>
                      <Pie data={qualityData} cx="50%" cy="50%" innerRadius={55} outerRadius={82} dataKey="value" label={({name,value})=>`${name}: ${value}%`}>
                        {qualityData.map((e,i)=><Cell key={i} fill={e.fill} />)}
              ...
              {missingData.length>0 && (
                <Card>
                  <SectionTitle icon={<AlertTriangle size={15} />} label="MISSING VALUES BY COLUMN" color={t.accentRed} />
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={missingData} margin={{ top:5,right:20,left:0,bottom:40 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke={dark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.05)"} />
                      <XAxis dataKey="name" tick={{ fill:t.sub, fontSize:11 }} angle={-35} textAnchor="end" />
              ...
              {relData.length>0 && (
                <Card>
                  <SectionTitle icon={<Link2 size={15} />} label="COLUMN RELATIONSHIPS" color={t.accent2} />
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={relData} layout="vertical" margin={{ top:5,right:30,left:90,bottom:5 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke={dark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.05)"} />
                      <XAxis type="number" tick={{ fill:t.sub, fontSize:11 }} />
              ...
              {numStats.length>0 && (
                <Card>
                  <SectionTitle icon={<Hash size={15} />} label="NUMERIC COLUMN STATISTICS" color={t.accentGreen} />
                  <ResponsiveContainer width="100%" height={260}>
                    <BarChart data={numStats} margin={{ top:5,right:20,left:0,bottom:40 }}>
                      <CartesianGrid strokeDasharray="3 3" stroke={dark?"rgba(255,255,255,0.04)":"rgba(0,0,0,0.05)"} />
                      <XAxis dataKey="name" tick={{ fill:t.sub, fontSize:11 }} angle={-35} textAnchor="end" />
            ...
            <div className="fade-up">
              <Card style={{ padding:"0", overflow:"hidden" }}>
                <div style={{ padding:"18px 22px", borderBottom:`1px solid ${t.border}`, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"10px" }}>
                  <div>
                    <div style={{ fontSize:"15px", fontWeight:700, color:t.text }}>Data Dictionary</div>
                    <div style={{ fontSize:"12px", color:t.muted, marginTop:"2px" }}>{filteredMeta.length} columns · Click any row for AI analysis</div>
                  </div>
                  <div style={{ display:"flex", gap:"10px", alignItems:"center", flexWrap:"wrap" }}>
                    <input placeholder="Search columns..." value={search} onChange={e=>setSearch(e.target.value)}
                      style={{ padding:"8px 14px", borderRadius:"8px", border:`1px solid ${t.border}`, background:t.input, color:t.text, fontSize:"13px", width:"160px" }} />
                    {sensitiveColumns.length > 0 && (
                      <button onClick={()=>setShowOnlyPII(!showOnlyPII)}
                        style={{ display:"flex", alignItems:"center", gap:"6px", padding:"8px 12px", border:`1px solid ${showOnlyPII?"rgba(239,68,68,0.5)":t.border}`, borderRadius:"8px", background:showOnlyPII?"rgba(239,68,68,0.12)":t.card, color:showOnlyPII?"#EF4444":t.sub, cursor:"pointer", fontSize:"12px", fontWeight:600 }}>
                        <Lock size={13} /> {showOnlyPII?"Show All":"PII Only"}
                      </button>
                    )}
                    <button onClick={downloadCSV} style={{ display:"flex", alignItems:"center", gap:"6px", padding:"8px 16px", border:"none", borderRadius:"8px", background:t.accent, color:"#fff", cursor:"pointer", fontSize:"13px", fontWeight:600 }}><Download size={14} /> Export</button>
                  </div>
                </div>

                {selectedCol && (
                  <div style={{ margin:"16px 22px", background:colInsightIsPII?"rgba(239,68,68,0.06)":dark?"rgba(139,92,246,0.08)":"rgba(139,92,246,0.05)", border:`1px solid ${colInsightIsPII?"rgba(239,68,68,0.25)":t.borderAccent}`, borderRadius:"12px", padding:"14px 16px" }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:"8px", alignItems:"center" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                        <span style={{ display:"flex", alignItems:"center", gap:"6px", fontSize:"13px", color:colInsightIsPII?"#EF4444":t.accent2, fontWeight:600, fontFamily:"'JetBrains Mono',monospace" }}><SearchIcon size={14} /> {selectedCol}</span>
                        {colInsightIsPII && <PIIBadge />}
                      </div>
                      <button onClick={()=>{setSelectedCol(null);setColInsight("");}} style={{ background:"none", border:"none", color:t.muted, cursor:"pointer", display:"flex" }}><X size={16} /></button>
                    </div>
                    {colInsightLoading
                      ? <div style={{ color:t.muted, fontSize:"13px", animation:"pulse 1.5s infinite" }}>Analyzing column...</div>
                      : <div style={{ fontSize:"13px", color:t.text, lineHeight:"1.7" }}>{colInsight}</div>}
                  </div>
                )}
                ...
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse" }}>
                    <thead>
                      <tr style={{ background:dark?"rgba(255,255,255,0.03)":"rgba(0,0,0,0.03)" }}>
                        {["Column","Type","PII","Description","Business Meaning","Role","Missing","Unique"].map(h=>(
                          <th key={h} style={{ padding:"11px 16px", textAlign:"left", borderBottom:`1px solid ${t.border}`, fontFamily:"'JetBrains Mono',monospace", fontSize:"10px", color:t.muted, fontWeight:600, letterSpacing:"0.06em", whiteSpace:"nowrap" }}>{h.toUpperCase()}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredMeta.map((m,i)=>(
                        <tr key={i} className={m.isPII ? "col-row pii-row" : "col-row"} onClick={()=>fetchColInsight(m.column)} style={{ borderBottom:`1px solid ${t.border}` }}>
                          <td style={{ padding:"12px 16px", fontFamily:"'JetBrains Mono',monospace", fontSize:"12px", color:t.text, fontWeight:600 }}>{m.column}</td>
                          <td style={{ padding:"12px 16px" }}><Badge label={m.type} color={typeColor(m.type)} /></td>
                          <td style={{ padding:"12px 16px" }}>{m.isPII ? <PIIBadge /> : <span style={{ fontSize:"11px", color:t.muted }}>—</span>}</td>
                          <td style={{ padding:"12px 16px", fontSize:"13px", color:t.sub, maxWidth:"220px", lineHeight:"1.5" }}>{m.description}</td>
                          <td style={{ padding:"12px 16px", fontSize:"12px", color:t.muted, maxWidth:"180px", lineHeight:"1.5" }}>{m.businessMeaning}</td>
                          <td style={{ padding:"12px 16px" }}><Badge label={m.relationship} color={relColor(m.relationship)} /></td>
                          <td style={{ padding:"12px 16px", fontFamily:"'JetBrains Mono',monospace", fontSize:"12px", color:parseFloat(m.stats?.missingPct)>10?t.accentRed:t.accentGreen, fontWeight:600 }}>{m.stats?.missingPct}%</td>
          ...
          {summary && tab==="chat" && !funTab && (
            <div className="fade-up" style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 130px)" }}>
              <Card style={{ flex:1, display:"flex", flexDirection:"column", padding:"0", overflow:"hidden" }}>
                <div style={{ padding:"16px 22px", borderBottom:`1px solid ${t.border}`, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
                    <div style={{ width:"36px", height:"36px", borderRadius:"50%", background:"#8B5CF6", display:"flex", alignItems:"center", justifyContent:"center" }}><Brain size={17} color="#fff" /></div>
                    <div>
                      <div style={{ fontSize:"14px", fontWeight:600, color:t.text }}>AI Assistant</div>
                      <div style={{ fontSize:"11px", color:t.accentGreen }}>Active · {summary.datasetName}{sensitiveColumns.length>0?` · ${sensitiveColumns.length} PII masked`:""}</div>
                    </div>
                  </div>
                ...
                {messages.length<=1 && (
                  <div style={{ padding:"14px 22px", borderBottom:`1px solid ${t.border}`, display:"flex", gap:"8px", flexWrap:"wrap" }}>
                    {["Show data type distribution","What columns have missing values?","What are the key patterns?","Which columns need cleaning?","Show numeric column stats"].map(q=>(
                      <button key={q} className="suggest-btn"
                        onClick={()=>{ if(textareaRef.current){ textareaRef.current.value=q; inputRef.current=q; textareaRef.current.focus(); } }}
                        style={{ padding:"6px 12px", border:`1px solid ${t.border}`, borderRadius:"20px", background:"transparent", color:t.muted, fontSize:"12px", cursor:"pointer" }}>{q}</button>
                    ))}
                ...
                <div style={{ flex:1, overflowY:"auto", padding:"20px 22px", display:"flex", flexDirection:"column", gap:"16px" }}>
                  {messages.map((msg,i)=>(
                    <div key={i} style={{ display:"flex", justifyContent:msg.role==="user"?"flex-end":"flex-start", animation:i===messages.length-1?"fadeUp 0.3s ease":"none" }}>
                      {msg.role==="model" && (
                        <div style={{ width:"28px", height:"28px", borderRadius:"50%", background:"#8B5CF6", display:"flex", alignItems:"center", justifyContent:"center", marginRight:"10px", flexShrink:0, marginTop:"4px" }}><Brain size={13} color="#fff" /></div>
                      )}
                      <div style={{ maxWidth:"80%", padding:"14px 18px", borderRadius:msg.role==="user"?"16px 16px 4px 16px":"16px 16px 16px 4px", background:msg.role==="user"?t.chatUser:t.chatAI, border:`1px solid ${t.border}`, color:t.text }}>
                        <SmartMessage content={msg.content} chartData={msg.chartData} dark={dark}
                          onSpeak={msg.role==="model" ? (text)=>speakText(text,i) : null}
                  ...
                  {chatLoading && (
                    <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                      <div style={{ width:"28px", height:"28px", borderRadius:"50%", background:"#8B5CF6", display:"flex", alignItems:"center", justifyContent:"center" }}><Brain size={13} color="#fff" /></div>
                      <div style={{ display:"flex", gap:"5px", padding:"12px 16px", background:t.chatAI, border:`1px solid ${t.border}`, borderRadius:"16px 16px 16px 4px" }}>
                        {[0,1,2].map(d=><div key={d} style={{ width:"7px", height:"7px", borderRadius:"50%", background:t.accent, animation:`pulse 1.3s ease ${d*0.2}s infinite` }} />)}
                      </div>
                    </div>
                ...
                <div style={{ padding:"16px 22px", borderTop:`1px solid ${t.border}`, display:"flex", gap:"10px", alignItems:"flex-end" }}>
                  <textarea ref={textareaRef} onChange={e=>{ inputRef.current=e.target.value; }}
                    onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); sendMessage(); } }}
                    placeholder={isListening?"Listening... speak now!":"Type or use the mic to speak your question..."}
                    rows={3} autoComplete="off" autoCorrect="off" spellCheck="false"
                    style={{ flex:1, padding:"12px 16px", borderRadius:"12px", border:`1px solid ${isListening?"#EF4444":t.border}`, background:isListening?"rgba(239,68,68,0.05)":t.input, color:t.text, fontSize:"14px", resize:"none", fontFamily:"'Inter',sans-serif", lineHeight:"1.5", transition:"all 0.2s" }}
                    onFocus={e=>e.target.style.borderColor=isListening?"#EF4444":t.borderAccent}
                    onBlur={e=>e.target.style.borderColor=isListening?"#EF4444":t.border}
                  ...
                  <button onClick={startListening} title={isListening?"Click to stop":"Click to speak"}
                    className={isListening?"mic-btn-active":""}
                    style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"12px 14px", borderRadius:"12px", background:isListening?"#EF4444":"rgba(139,92,246,0.15)", color:isListening?"#fff":t.accent2, cursor:"pointer", flexShrink:0, transition:"all 0.2s", border:`1px solid ${isListening?"#EF4444":t.borderAccent}` }}>
                    {isListening?<MicOff size={18} />:<Mic size={18} />}
                  </button>
                  <button className="send-btn" onClick={sendMessage} disabled={chatLoading}
                    style={{ display:"flex", alignItems:"center", justifyContent:"center", padding:"12px 20px", border:"none", borderRadius:"12px", background:"#8B5CF6", color:"#fff", cursor:"pointer", fontWeight:700, opacity:chatLoading?0.4:1, flexShrink:0 }}><Send size={18} /></button>
                </div>
              </Card>
            </div>
          )}
          ...
          {summary && funTab==="detective" && (
            <div className="fade-up">
              <Card style={{ borderColor:"rgba(239,68,68,0.3)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"20px" }}>
                  <div style={{ width:"48px", height:"48px", borderRadius:"14px", background:"rgba(239,68,68,0.15)", border:"1px solid rgba(239,68,68,0.3)", display:"flex", alignItems:"center", justifyContent:"center" }}><SearchIcon size={22} color="#EF4444" /></div>
                  <div>
                    <div style={{ fontSize:"17px", fontWeight:700, color:t.text }}>Data Detective</div>
                    <div style={{ fontSize:"12px", color:"#EF4444" }}>Case File — {summary.datasetName}</div>
                  </div>
                ...
                {detectiveLoading
                  ? <div style={{ color:t.muted, fontSize:"14px", animation:"pulse 1.5s infinite", padding:"30px 0", textAlign:"center" }}>Detective is investigating your data...</div>
                  : <div style={{ fontSize:"14px", color:t.text, lineHeight:"1.9", whiteSpace:"pre-wrap", padding:"20px", background:"rgba(239,68,68,0.05)", borderRadius:"14px", border:"1px solid rgba(239,68,68,0.15)" }}>{detectiveReport}</div>
                }
                <button onClick={fetchDetective} style={{ marginTop:"16px", padding:"10px 22px", borderRadius:"10px", background:"rgba(239,68,68,0.15)", color:"#EF4444", cursor:"pointer", fontSize:"13px", fontWeight:600, border:"1px solid rgba(239,68,68,0.3)" }}>
                  Reinvestigate
                </button>
          ...
          {summary && funTab==="horoscope" && (
            <div className="fade-up">
              <Card style={{ borderColor:"rgba(167,139,250,0.3)", background:dark?"rgba(167,139,250,0.03)":"rgba(167,139,250,0.06)" }}>
                <div style={{ display:"flex", alignItems:"center", gap:"12px", marginBottom:"20px" }}>
                  <div style={{ width:"48px", height:"48px", borderRadius:"14px", background:"rgba(167,139,250,0.15)", border:"1px solid rgba(167,139,250,0.3)", display:"flex", alignItems:"center", justifyContent:"center" }}><Sparkles size={22} color="#A78BFA" /></div>
                  <div>
                    <div style={{ fontSize:"17px", fontWeight:700, color:t.text }}>Data Horoscope</div>
                    <div style={{ fontSize:"12px", color:"#A78BFA" }}>Cosmic Reading — {summary.datasetName}</div>
                  </div>
                ...
                {horoscopeLoading
                  ? <div style={{ color:t.muted, fontSize:"14px", animation:"pulse 1.5s infinite", padding:"30px 0", textAlign:"center" }}>Reading the stars for your dataset...</div>
                  : <div style={{ fontSize:"14px", color:t.text, lineHeight:"1.9", whiteSpace:"pre-wrap", padding:"20px", background:"rgba(167,139,250,0.07)", borderRadius:"14px", border:"1px solid rgba(167,139,250,0.2)" }}>{horoscope}</div>
                }
                <button onClick={fetchHoroscope} style={{ marginTop:"16px", padding:"10px 22px", borderRadius:"10px", background:"rgba(167,139,250,0.15)", color:"#A78BFA", cursor:"pointer", fontSize:"13px", fontWeight:600, border:"1px solid rgba(167,139,250,0.3)" }}>
                  Read Again
                </button>
